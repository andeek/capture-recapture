...
diff_df %>% ungroup() %>%
  ...
  ggplot() +
  geom_point(aes(truth_clust_diff/truth*100, mean_diff, colour = crc_method, shape = method), alpha = .4) +
  geom_density_2d(aes(truth_clust_diff/truth*100, mean_diff), colour = "black", data = diff_df %>% ungroup() %>% filter(num_include == 1 & crc_method == "BMA") %>% mutate(num_include = factor(num_include, labels = paste0("Number db = ", unique(num_include)))) %>% mutate(num = factor(num, labels = paste0("Num. distorted = ", unique(num)), levels = sort(as.numeric(unique(num)))))) +
  geom_density_2d(aes(truth_clust_diff/truth*100, mean_diff), colour = "grey30", data = diff_df %>% ungroup() %>% filter(num_include == 1 & crc_method == "NPLCM") %>% mutate(num_include = factor(num_include, labels = paste0("Number db = ", unique(num_include)))) %>%
  mutate(num = factor(num, labels = paste0("Num. distorted = ", unique(num)), levels = sort(as.numeric(unique(num)))))) +
