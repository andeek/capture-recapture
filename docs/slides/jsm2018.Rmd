...
## truth
make_density_df <- function(dist, num, crc, method, pop_N, min, max, n_bin) {
  N <- pop_N %>% filter(crc_method == crc & num == num & method == method) %>% .$N
  df <- data.frame(dist = dist, num = num, crc_method = crc, data.frame(density(N, from = min, to = max, n = n_bin)[c("x", "y")]))
  return(df)
}
rbind(make_density_df(5, 1, "NPLCM", "Truth", pop_N_5, min_tot, max_tot, n_bin),
...
## add mmpms
rbind(make_density_df(5, 1, "NPLCM", "Shared MPMMS", pop_N_5, min_tot, max_tot, n_bin),
...
## add joint posterior
rbind(make_density_df(5, 1, "NPLCM", "U(N)", pop_N_5, min_tot, max_tot, n_bin),
...
##bma
## truth
rbind(make_density_df(5, 1, "BMA", "Truth", pop_N_5, min_tot, max_tot, n_bin),
...
## add mmpms
rbind(make_density_df(5, 1, "BMA", "Shared MPMMS", pop_N_5, min_tot, max_tot, n_bin),
...
## add joint posterior
rbind(make_density_df(5, 1, "BMA", "U(N)", pop_N_5, min_tot, max_tot, n_bin),
...
```{r plot-results}
pop_N_density %>%
  gather(method, y, -dist, -num, -x, -crc_method) %>% 
  mutate(dist = paste0("Dist. level = ", dist),
...
pop_N_5 %>%
  group_by(dist, num, crc_method, method) %>%
  summarise(mean_N = mean(N), q05_N = quantile(N, probs = 0.025), q95_N = quantile(N, probs = 0.975), MSE = mean((N - 1000)^2)) -> pop_N_5_summary_stats

pop_N_5_summary_stats %>% 
  group_by(method, add = TRUE) %>% 
  transmute(bold = 1000 >= q05_N & 1000 <= q95_N) %>%
  arrange(dist, num, method, crc_method) -> which_bold

pop_N_5_summary_stats %>% 
  arrange(dist, num, method, crc_method) %>% 
  ...
  kable(format = "html", row.names = FALSE) %>%
  row_spec(which(which_bold %>% .$bold), bold = TRUE)
```
...
pop_N_10 %>%
  group_by(dist, num, crc_method, method) %>%
  summarise(mean_N = mean(N), q05_N = quantile(N, probs = 0.025), q95_N = quantile(N, probs = 0.975), MSE = mean((N - 1000)^2)) -> pop_N_10_summary_stats

pop_N_10_summary_stats %>% 
  group_by(method, add = TRUE) %>% 
  transmute(bold = 1000 >= q05_N & 1000 <= q95_N) %>%
  arrange(dist, num, method, crc_method) -> which_bold
...
pop_N_15 %>%
  group_by(dist, num, crc_method, method) %>%
  summarise(mean_N = mean(N), q05_N = quantile(N, probs = 0.025), q95_N = quantile(N, probs = 0.975), MSE = mean((N - 1000)^2)) -> pop_N_15_summary_stats
...
## get MSE for N
## truth
pop_N_5 %>% filter(num == 3) %>%
  bind_rows(pop_N_10 %>% filter(num == 3)) %>%
  bind_rows(pop_N_15 %>% filter(num == 3)) %>%
  filter(method == "Truth" & num == 3) %>%
...
data.frame(dist = 5, method = "U(N)", crc_method = "NPLCM", 
           mean_diff = apply(crc_bayes_5, 2, function(N) mean(N - 1000))) %>%
  mutate(iter = 1:n()) %>%
  bind_rows(data.frame(dist = 5, method = "U(N)", crc_method = "BMA", 
           mean_diff = apply(bma_bayes_5, 2, function(N) mean(N - 1000))) %>%
             mutate(iter = 1:n())) %>%
  bind_rows(data.frame(dist = 10, method = "U(N)", crc_method = "NPLCM", 
           mean_diff = apply(crc_bayes_10, 2, function(N) mean(N - 1000))) %>%
             mutate(iter = 1:n())) %>%
  bind_rows(data.frame(dist = 10, method = "U(N)", crc_method = "BMA", 
           mean_diff = apply(bma_bayes_10, 2, function(N) mean(N - 1000))) %>%
             mutate(iter = 1:n())) %>%
    bind_rows(data.frame(dist = 15, method = "U(N)", crc_method = "NPLCM", 
           mean_diff = apply(crc_bayes_15, 2, function(N) mean(N - 1000))) %>%
             mutate(iter = 1:n())) %>%
  bind_rows(data.frame(dist = 15, method = "U(N)", crc_method = "BMA", 
           mean_diff = apply(bma_bayes_15, 2, function(N) mean(N - 1000))) %>%
