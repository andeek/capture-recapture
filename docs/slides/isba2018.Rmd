...
- MCMC for estimation of $U(\eta)$ and $\text{Var} (\eta \mid \boldsymbol X)$ is **computationally expensive**, but using the shared MPMMS does not work very well. Explore how to propogate the error approximately.

- NPLCM CRC method is for independent lists. In our (limited) experience, this CRC method works best for many lists ( $\ge 4$ ). Is sensitive to errors from RL.

- BMA CRC method does not scale beyond $5$ lists easily because of the need to precompute all valid dependency structures between lists. Is also sensitive to errors from RL. 
