...
chaperones <- function(Data, N, Prior, x1, init, x, z, id, cat_fields, string_fields, 
                       ...
                       betas, lods, upds, hpriords, samind, rep, out.dir, eval = FALSE){
  nfields <- ncol(x)
  ...
  for (i in 1:(burn + nsamples)) {
    ...
    if(i %% thin == 0) {
      ...
      cat(z, "\n", file=sprintf("%s/lambda_%s_%s_%s.txt", out.dir, Prior, Data, rep), sep=" ", append=TRUE)
  ...
  if(eval) write.csv(data.frame(Initial=param.to.save, Mean=post_mean_dpm, SD=post_sd_dpm, FNR=meanRates[1], FDR=meanRates[2], SamplerTime_Min=runtime[3]/60), sprintf("%s/LinkRates_%s_%s_%s.csv", out.dir, Prior, Data, rep), row.names=FALSE)
}
